...
class Backend(odoo.addons.web.controllers.main.Home):
    @http.route('/backend/template_get_modify', type='json', auth='user', website=True)
    def backend_template_get_modify(self, key, full=False, bundles=False):
        ...
        """
        return request.env["ir.ui.view"].template_get_modify(key, full=full, bundles=bundles)
